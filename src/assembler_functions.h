...
typedef struct ass_prog_t{
    ...
    char flag_names[(MAX_FLAG_NAME_SIZE + 1) * MAX_FLAG_NUM];
    size_t flags[MAX_FLAG_NUM];
    size_t flag_num;

    enum ass_errors error;
    FILE *log_file;
} ass_prog_t;

void devide_in_lines(ass_prog_t *ass_prog);

enum ass_registers get_reg_num(const char *reg, ass_prog_t *ass_prog);
