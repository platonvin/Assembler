...
{
    if(stack->should_log) fprintf(stack->log_file, "In func %s, line %d\nsize = %zu, capacity = %zu\n", func_name, line, stack->size, stack->capacity);

    if(stack->data != NULL)
    {
        for(size_t i = 0; i < stack->size + 2; i++)
        {
            if(stack->should_log) fprintf(stack->log_file, "%d ", stack->data[i]);
        }
        if(stack->should_log) fprintf(stack->log_file, "\nhash = %zu ", stack->hash);
        if(stack->should_log) fprintf(stack->log_file, "get_data_hash = %zu ", get_data_hash(stack));
        if(stack->should_log) fprintf(stack->log_file, "get_hash = %zu (%d)\n\n", get_hash(stack->data[stack->size]), stack->data[stack->size]);
    }
    else 
    {
        if(stack->should_log) fprintf(stack->log_file, "\ndata = NULL\n\n");
